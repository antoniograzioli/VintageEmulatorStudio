...
if _OPTIONS["vs"]==nil then
		buildoptions {
			...
			"/wd4703", -- warning C4703: potentially uninitialized local pointer variable used
		}
end
...
-- LUA library objects
--------------------------------------------------
-- vesembedded is a standalone target, but its AU link configuration consumes
-- the bundled Lua, lualibs and SQLite static archives.  Keep those projects
-- in its generated build graph so a clean, relocated build produces them.
if (STANDALONE~=true) or (_OPTIONS["subtarget"] == "vesembedded") then
...
if _OPTIONS["vs"]==nil then
		buildoptions {
			...
			"/wd4310", -- warning C4310: cast truncates constant value
			"/wd4701", -- warning C4701: potentially uninitialized local variable 'xxx' used
			"/wd4828", -- warning C4828: vendored Dear ImGui markdown text contains non-UTF-8 bytes
		}
elseif _OPTIONS["vs"]=="clangcl" then
		buildoptions {
			"-Wno-uninitialized",
